...
func main() {
	var (
		Addr               = flag.String("p", "127.0.0.1:80", "server_IP") // 字符串类型
		request_packet_num = flag.String("n", "1", "request_packet_num")
	...
	addr := Addr
	req := httptest.NewRequest("GetN ", fmt.Sprintf("https://%s/helloworld", addr), nil)
